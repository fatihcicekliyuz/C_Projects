#include <stdio.h>
int fun(int *gelen,int b,int gelen_sayi) //bu kýsýmda fonksiyon ile verileri çekip sonrasýnda for döngüsü ile en büyük buluyoruz ve sonrasýnda ekrana yazdýrýyoruz
{
	printf("Adi    : Fatih \n"); 
...
int deger[1000]={};
int k=0; // for'un kaç kere döndüðünü hesaplamak için k deðiþkeni tanýmladým.

 for(int i=0;i<1000;i=i+sayi){   // kullanýcýnýn girdiði deðere göre ardýþýk çarpým yapýsý seçilir ve deger olarak tanýmlanan diziye sýrasýyla deðerler atanýr.
	switch (sayi)
    {
      case 2:
  ...
  fun(deger,k,sayi);// deðerler kullanýlmak uzere fonksiyone gönderilir
}
